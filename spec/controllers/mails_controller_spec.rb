...
describe MailsController do
  ...
  def mock_mail(stubs={})
    @mock_mail ||= mock_model(Mail, stubs)
  end

  describe "GET index" do
    it "assigns all mails as @mails" do
      Mail.stub(:find).with(:all).and_return([mock_mail])
      get :index
      assigns[:mails].should == [mock_mail]
    end
  end

  describe "GET show" do
    it "assigns the requested mail as @mail" do
      Mail.stub(:find).with("37").and_return(mock_mail)
      get :show, :id => "37"
      assigns[:mail].should equal(mock_mail)
    end
  end

  describe "GET new" do
    it "assigns a new mail as @mail" do
      Mail.stub(:new).and_return(mock_mail)
      get :new
      assigns[:mail].should equal(mock_mail)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created mail as @mail" do
        Mail.stub(:new).with({'these' => 'params'}).and_return(mock_mail(:save => true))
        post :create, :mail => {:these => 'params'}
        assigns[:mail].should equal(mock_mail)
      end

      it "redirects to the created mail" do
        Mail.stub(:new).and_return(mock_mail(:save => true))
        post :create, :mail => {}
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved mail as @mail" do
        Mail.stub(:new).with({'these' => 'params'}).and_return(mock_mail(:save => false))
        post :create, :mail => {:these => 'params'}
        assigns[:mail].should equal(mock_mail)
      end

      it "re-renders the 'new' template" do
        Mail.stub(:new).and_return(mock_mail(:save => false))
        post :create, :mail => {}
  ...
  describe "PUT update" do
    ...
    describe "with valid params" do
      it "updates the requested mail" do
        Mail.should_receive(:find).with("37").and_return(mock_mail)
        mock_mail.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => "37", :mail => {:these => 'params'}
      end

      it "assigns the requested mail as @mail" do
        Mail.stub(:find).and_return(mock_mail(:update_attributes => true))
        put :update, :id => "1"
        assigns[:mail].should equal(mock_mail)
      end
    ...
    describe "with invalid params" do
      ...
      it "assigns the mail as @mail" do
        Mail.stub(:find).and_return(mock_mail(:update_attributes => false))
        put :update, :id => "1"
        assigns[:mail].should equal(mock_mail)
      end
  ...
  describe "DELETE destroy" do
    ...
    it "redirects to the mails list" do
      Mail.stub(:find).and_return(mock_mail(:destroy => true))
      delete :destroy, :id => "1"
